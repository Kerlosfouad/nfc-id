              ...
              {profile.links.map((link, i) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 sm:gap-3 border rounded-xl px-2 sm:px-3 py-2 sm:py-2.5 group hover:border-white/20 transition-all cursor-pointer ${isHidden ? "bg-white/2 border-white/5 opacity-60" : "bg-white/5 border-white/10"}`}
                  >
                    {/* Drag handle */}
                    <i className="ri-drag-move-2-line text-white/20 text-sm sm:text-base group-hover:text-white/50 transition-colors cursor-grab active:cursor-grabbing flex-shrink-0 hidden sm:block" />

                    {/* Icon */}
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: m.color + "20" }}>
                      <i className={m.icon + " text-sm"} style={{ color: m.color }} />
                    </div>
                    ...
                    <button
                      ...
                      className={`relative w-9 h-5 rounded-full transition-all flex-shrink-0 ${isHidden ? "bg-white/10" : "bg-[#03A9F4]"}`}
  ...
  return (
    <div className="flex h-screen bg-[#111] text-white overflow-hidden" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <main className="flex-1 overflow-hidden flex flex-col min-w-0">
        {/* Mobile top bar */}
        <div className="md:hidden flex items-center gap-3 px-4 py-3 border-b border-white/5 bg-[#0f0f0f] flex-shrink-0">
