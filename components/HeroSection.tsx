...
export default function HeroSection() {
  ...
  return (
    <section dir={isArabic ? "rtl" : "ltr"} className={`relative flex justify-center items-center overflow-hidden ${isArabic ? "font-[Cairo]" : ""}`} style={{ minHeight: "100svh" }}>
      ...
      <div className="absolute bottom-1/4 right-1/4 w-[300px] h-[300px] rounded-full bg-[#0066ff]/3 blur-[90px] pointer-events-none z-[1] animate-pulse-slow2" />
          ...
          <div className="grid grid-cols-3 gap-2.5 sm:flex sm:items-center sm:justify-center sm:gap-8">
            {stats.map((s, i) => (
              <div key={i} className="text-center">
                ...
                <div className="text-[10px] sm:text-xs text-white/60 uppercase tracking-wide sm:tracking-wider leading-tight">{s.label}</div>
              </div>
            ))}
          </div>
